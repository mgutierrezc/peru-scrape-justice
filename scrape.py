...
default_temp_download_folder = os.path.join(
    os.path.realpath(os.path.dirname(__file__)), "busquedaform_temp_downloads"
)

faulty_downloads_dir = os.path.join(
    os.path.realpath(os.path.dirname(__file__)), "busquedaform_faulty_downloads"
)
final_data_folder = os.path.join(
    os.path.realpath(os.path.dirname(__file__)), "data", "busquedaform_data"
)
...
def retry_download(link_el, max_tries, target_download_dir, driver):
    ...
    while tries < max_tries and not success:
        ...
        if len(file_names) > 0:
            temp_file_path = os.path.join(default_temp_download_folder, file_names[0])
            shutil.move(temp_file_path, target_download_dir)
