...
GLOBAL_DIR = os.getenv(r"GLOBAL_DIR")
...
def rename_partially_downloaded_files():
    ...
    for filename in partial_files:
        ...
        new_filename = (
            filename[0:start_index]
            + "crdownload_"
            + filename[start_index : filename.rindex(".")]
        )
        os.rename(
            filename, new_filename
        )  # removing .crdownload extension from files, but adding a prefix crdownload_ to such files
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Clean & extract data into csv files from scraped files"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="*",
        choices=list(range(1978, current_year + 1)),
        default=None,
...
if __name__ == "__main__":
    license_path = os.getenv(r"LICENSE_PATH")
    ...
    for index in [2022]:
        ...
        with codecs.open(f"{out_dir}/DF_INTEGRITY_2022.csv", "w", ENCODING) as fp:
            ...
            for index in tqdm.trange(len(list_files)):
                ...
                if text != None:
                    code_from_text = search_regexes(
                        text,
                        [r"\d*-\d*-\d*-\d*-\d*\w*-\w*-\d*", r"\d*-\d*-\d*-\w*-\w*-\d*"],
                    )
